...
/* 
flag=0 represents only xy plane
flag=1 represents only yz plane
flag=2 represents rest all cases
*/
...
void input(point a,point b,point c,ifstream &in)
{
   ...
   else
   {
    ...
    else if(abs(p.C)>abs(p.B) && abs(p.C)>abs(p.A)) flag=2;
    else if(abs(p.A)==abs(p.B) && abs(p.A)>abs(p.C)) flag=4;
    else if(abs(p.A)>abs(p.B) && abs(p.A)==abs(p.C)) flag=3;
    else if(abs(p.C)==abs(p.B) && abs(p.A)<abs(p.C)) flag=5;
    else flag=6;
