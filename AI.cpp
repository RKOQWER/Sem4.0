...
yz - blue   -1
zx - green  -2 
2 functional plane - black -3
3 functional plane - red  -4
No fuunctional plane - white -5
*/
...
void coloring()
{
   ...
   cout<<"Count of number of voxels : "<<cnt<<"\n";
}
bool digital_line(point a,point b,point c)
{
   /*
      ...
      for(i=0;i<pixels.size();i++)
         {
            ...
            int p=(int)z;
            if(ptr[x-x_l][y-y_l][p-z_l]==-1)
            {
               ptr[x-x_l][y-y_l][p-z_l]=0;
            }
         }
   }
   else if(flag==1)
      ...
      for(i=0;i<pixels.size();i++)
         {
            ...
            if(A>0)
               ...
               x=ceil(x);
            int p=(int)(x);
            if(ptr[p-x_l][y-y_l][z-z_l]==-1)
            {
               ptr[p-x_l][y-y_l][z-z_l]=1;
            }
         }
   }
   else
      ...
      for(i=0;i<pixels.size();i++)
         {
            ...
            if(B>0)
               ...
               y=ceil(y);
            int p=(int)(y);
            if(ptr[x-x_l][p-y_l][z-z_l]==-1)
            {
               ptr[x-x_l][p-y_l][z-z_l]=2;
            }
         }
   }
...
void dst(vector<pair<int,int> > &v1,vector<pair<int,int> > &v2,int flag)
{
   ...
   if(flag==0)// for each x;
   {
      ...
      for(int i=0;i<v2.size();i++)
      {
         ...
         int x=v2[i].second;
         int y=mpp[x];
         if(ptr[x-x_l][y-y_l][z-z_l]==-1)
         {  
            ptr[x-x_l][y-y_l][z-z_l]=0;
         }
      }
      mpp.clear();
   }
...
void input(point a,point b,point c,ifstream &in)
{
   ...
   // Check for single point
      ...
      {
         if(ptr[a.x-x_l][a.y-y_l][a.z-z_l]==-1)
         {
...
{
    scaling();
    ptr.resize(x_m-x_l+1,vector<vector<int> >(y_m-y_l+1,vector<int>(z_m-z_l+1,-1)));
    ifstream in;  
      in.open("mcube.obj"); 
      if(!in) { 
         cerr << "Error: file could not be opened" << endl;
