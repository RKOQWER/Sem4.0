...
yz - blue
zx - green
2 functional plane - black
3 functional plane - red
No fuunctional plane - white
*/
...
void visualisation(ofstream &outdata,point q,int &count,int &g);
...
void bfs(int i,int j,int k,ofstream &outdata,int g,float &fx,float &fy,float &fz,ofstream &out
   ,int &count);
int x_l=INT_MAX;
int y_l=INT_MAX;
int z_l=INT_MAX;
   ...
   int &count)
{
   ...
   while(!q.empty())
   {
      ...
      while(sz--)
      {
         point f=q.front();
         gc++;
         visualisation(outdata,f,count,g);
         q.pop();
         for(int x=-1;x<=1;x+=2)
         {
            for(int y=-1;y<=1;y+=2)
            {
               for(int z=-1;z<=1;z+=2)
               {
                  if(validate(f.x+x,f.y+y,f.z+z))
                  {
                     vis[f.x+x][f.y+y][f.z+z]=true;
                     point p;
                     p.x=f.x+x;
                     p.y=f.y+y;
                     p.z=f.z+z;
...
void coloring()
{
   ...
   ofstream out;
   out.open("color.mtl"); // opens the file
   if( !out ) { // file couldn't be opened
      cerr << "Error: file could not be opened" << endl;
      exit(1);
   }
   outdata<<"mtllib color.mtl\n";
   int cnt=0;// stores number of voxels
   int g=0;// stores number of groups
   int i,j,k;
   int count=1;
   ...
   for(i=0;i<x_m-x_l+1;i++)
   {
      for(j=0;j<y_m-y_l+1;j++)
      {
         ...
         {
            if(ptr[i][j][j] && !vis[i][j][k])
            {
               g++;
               cnt++;
               cout<<i<<" "<<j<<" "<<k<<" "<<1<<"\n";
               bfs(i,j,k,outdata,g,fx,fy,fz,out,count);
               fx+=0.1;
               fy+=0.3;
               fz+=0.5;
               // cout<<"Kd "<<fx<<" "<<fy<<" "<<fz<<"\n";
   ...
   cout<<"Number of voxels = "<<cnt<<"\n";
   cout<<"Number of groups = "<<g<<"\n";
   cout<<"Count of voxels in group "<<gc<<"\n";
   outdata.close();
   out.close();
}
bool digital_line(point a,point b,point c)
...
int main()
{
   ...
   while ( !in.eof() )
   {
         ...
         else// a b c stores the 3 vertices corresponding to faces
         {
            point a,b,c;
